...
public class MainActivity extends AppCompatActivity {
    ...
    public void Button1OnClicked(View v){
        try {
            ...
            if(database!=null){
                ...
                database.execSQL("INSERT INTO " + "PORODUCT" + "(username, password) VALUES" +
                        "("+"'"+UserName+"'"+","+"'"+Password+"'"+");");
    ...
    public void Button2OnClicked(View v){
        try {
            ...
            if(database!=null){
                Cursor cursor =database.rawQuery("SELECT username, password FROM " + "PORODUCT", null);

                int count = cursor.getCount();
                println("number of result records :"+count);
                for(int i=0;i<count;i++){
                    cursor.moveToNext();
                    String username = cursor.getString(0);
                    String password = cursor.getString(1);
                    println("record # "+i+": "+username+","+password);
                }
                cursor.close();
                println("bwe.");
            }
    ...
    public void login(View v) {
        if (database != null) {
            ...
            if (Text.equals(Cusername) && Text2.equals(Cpassword)) {//내장디비인 SQLite에서 가져온 이름, 전공과 입력한 이름, 전공을 비교하여 같으면 로그인이 되게한다.
                ...
                Toast.makeText(getApplication(),Text+"Welcome!",Toast.LENGTH_SHORT).show();
                finish();
